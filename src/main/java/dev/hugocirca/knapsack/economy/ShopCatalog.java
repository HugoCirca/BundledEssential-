...
/**
 * Single source of truth for shop category membership.
 * Previously duplicated between PriceManager (price values) and ShopManager (category lists).
 * Now ShopManager delegates here; PriceManager can optionally validate against these lists.
 * <p>
 * Keeps version-safe {@link #mats(String...)} helper so unknown materials on old servers are skipped.
 */
public final class ShopCatalog {

    public static final String[] SHOP_EXCLUDED = {
            "COMMAND_BLOCK", "CHAIN_COMMAND_BLOCK", "REPEATING_COMMAND_BLOCK",
            "COMMAND_BLOCK_MINECART", "STRUCTURE_BLOCK", "STRUCTURE_VOID", "JIGSAW",
            "BARRIER", "DEBUG_STICK", "KNOWLEDGE_BOOK", "LIGHT", "BEDROCK",
            "SPAWNER", "DRAGON_EGG",
            "BUDDING_AMETHYST", "CHORUS_PLANT", "DIRT_PATH", "END_PORTAL_FRAME", "AIR",
            "FARMLAND", "FROGSPAWN", "LARGE_AMETHYST_BUD", "MEDIUM_AMETHYST_BUD", "PETRIFIED_OAK_SLAB",
            "REINFORCED_DEEPSLATE", "SUSPICIOUS_GRAVEL", "SUSPICIOUS_SAND", "TEST_BLOCK", "TEST_INSTANCE_BLOCK", "TRIAL_SPAWNER", "VAULT"
    };

    private ShopCatalog() {}

    /** Version-safe resolver: unknown names on old servers are skipped with a warning. */
    public static Material[] mats(String... names) {
    ...
    public static Material icon(String name, Material fallback) {
        try {
    ...
    public static Map<String, Material[]> categories() {
        Map<String, Material[]> map = new LinkedHashMap<>();
        map.put("Logs", logs());
        map.put("Stone", stone());
        map.put("Ores", ores());
        map.put("Crops", crops());
        map.put("Mob Drops", mobDrops());
        ...
        map.put("End", end());
        map.put("New 1.21-26.2", latest());
        map.put("Misc", misc(map));
        return map;
    }

    /** Catch-all: every item not in another category and not excluded. */
    public static Material[] misc(Map<String, Material[]> already) {
        Set<Material> listed = new HashSet<>();
        for (Material[] arr : already.values()) listed.addAll(Arrays.asList(arr));
        Set<Material> excluded = new HashSet<>();
        for (String n : SHOP_EXCLUDED) {
            try { Material m = Material.matchMaterial(n); if (m != null) excluded.add(m); } catch (Exception ignored) {}
        }
        List<Material> rest = new ArrayList<>();
        for (Material m : Material.values()) {
            try {
                if (m == null || m == Material.AIR || !m.isItem()) continue;
                if (listed.contains(m) || excluded.contains(m)) continue;
    ...
    // ---- individual categories (copied verbatim from ShopManager, now centralized) ----

    public static Material[] logs() {
        return mats(
    ...
    public static Material[] mobDrops() {
        return mats(
    ...
    public static Material[] end() {
        return mats(
            ...
            "GRAY_SHULKER_BOX","LIGHT_GRAY_SHULKER_BOX","CYAN_SHULKER_BOX","PURPLE_SHULKER_BOX",
            "BLUE_SHULKER_BOX","BROWN_SHULKER_BOX","GREEN_SHULKER_BOX","RED_SHULKER_BOX","BLACK_SHULKER_BOX",
            "DRAGON_BREATH","DRAGON_HEAD","END_CRYSTAL","ELYTRA"
        );
    }

    public static Material[] latest() {
        return mats(
